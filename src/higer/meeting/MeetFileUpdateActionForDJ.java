...
public class MeetFileUpdateActionForDJ implements Action{
	/**
	...
	public String execute(RequestInfo info) {
		...
		if(rs.next()){
			...
			if(!"".equals(xgfj)) {
				//数据更新
				int idx = 0;
				sql = " select id as idx from formtable_main_103_dt1 where fzr = '"+scr+"' and mainid in (select id from formtable_main_103 where lcid = '"+hymc1+"')";
				rs.execute(sql);
				if(rs.next()){
					idx = rs.getInt("idx");
				}
				if(idx!=0) {
					//有发言人则更新发言人议程附件
					sql = " update formtable_main_103_dt1 set ycfj ='"+xgfj+"' where id ="+idx;
					rs.execute(sql);
					//log.writeLog("Update meeting Document 1---"+sql);
				}else {
					sql = " update formtable_main_103 set xgfj = xgfj||',"+xgfj+"' where lcid = '"+hymc1+"' ";
					rs.execute(sql);
