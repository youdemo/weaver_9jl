...
public class DemoActionTest {
    ...
    public String weatableDemo(@Context HttpServletRequest request, @Context HttpServletResponse response){
        ...
        try {
            ...
            apidatas.putAll(new DemoActionImplTest().getTables(user, ParamUtil.request2Map(request)));
    ...
    public String weatableConditonDemo(@Context HttpServletRequest request, @Context HttpServletResponse response){
        ...
        try {
            //获取当前用户
            User user = HrmUserVarify.getUser(request, response);
            apidatas.putAll(new DemoActionImplTest().getCondition(user, ParamUtil.request2Map(request)));
            apidatas.put("api_status", true);
        } catch (Exception e) {
            e.printStackTrace();
            apidatas.put("api_status", false);
