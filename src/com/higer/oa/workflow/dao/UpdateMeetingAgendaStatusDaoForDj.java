...
public class UpdateMeetingAgendaStatusDaoForDj {
	 /**
     * 日志.
     */
  //  private static Logger LOGGER = Logger.getLogger(UpdateMeetingAgendaStatusDaoForDj.class);
    ...
    /**
     	* 构造方法.
     */
    public UpdateMeetingAgendaStatusDaoForDj() {
        rs = new RecordSet();
    }
    /**
     	*  根据workflowid获取表名
    ...
    public List<Map<String,String>> getGLYt(String glyc,String ycids) {
    	List<Map<String,String>> list = new ArrayList<Map<String,String>>();
        rs.execute("select ytgl from formtable_main_100_dt1 where mainid="+glyc+" and id in("+ycids+")");
        while (rs.next()) {
        	Map<String,String> map = new HashMap<String,String>();
    ...
    public List<Map<String,String>> getWGLYt(String glyc,String ycids) {
    	List<Map<String,String>> list = new ArrayList<Map<String,String>>();
        rs.execute("select ytgl from formtable_main_100_dt1 where mainid="+glyc+" and id not in("+ycids+")");
        while (rs.next()) {
        	Map<String,String> map = new HashMap<String,String>();
        	map.put("ytgl", Util.null2String(rs.getString("ytgl")));
        	list.add(map);
    ...
    public void UpdateAgendaStatus(String glyc,String ycids) {
    	rs.execute("update formtable_main_100 set zzbs_new = '0' where id="+glyc);
    	rs.execute("update formtable_main_100_dt1 set zt = '0' where mainid="+glyc+" and id in("+ycids+")");
    	rs.execute("update formtable_main_100_dt1 set zt = '1' where mainid="+glyc+" and id not in("+ycids+")");
    }
    /**
     * 更新议题转台
     * @param ytids 议题id
    ...
    public void UpdateYtStatus(String ytids,String status) {
    	rs.execute("update formtable_main_104_dt1 set zt = '"+status+"' where id in("+ytids+")");
    }
